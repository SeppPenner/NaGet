...
namespace NaGet.Protocol.Tests
{
    ...
    {
        private readonly RawPackageMetadataClient target;

        public RawPackageMetadataClientTests(ProtocolFixture fixture)
        {
            target = fixture.MetadataClient;
        }

        [Fact]
        public async Task GetRegistrationIndexInlinedItems()
        {
            var result = await target.GetRegistrationIndexOrNullAsync("Test.Package");
        ...
        {
            var result = await target.GetRegistrationIndexOrNullAsync("Paged.Package");
        ...
        {
            var result = await target.GetRegistrationPageAsync(TestData.RegistrationPageUrl);
        ...
        {
            var result = await target.GetRegistrationLeafAsync(TestData.RegistrationLeafListedUrl);
        ...
        {
            var result = await target.GetRegistrationLeafAsync(TestData.RegistrationLeafUnlistedUrl);
