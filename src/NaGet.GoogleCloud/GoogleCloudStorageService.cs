...
public class GoogleCloudStorageService : IStorageService
{
    ...
    public async Task<Stream?> GetAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        await storage.DownloadObjectAsync(bucketName, CoercePath(path), stream, cancellationToken: cancellationToken);
        stream.Position = 0;
        return stream;
    }
    ...
    public async Task DeleteAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var obj = await storage.GetObjectAsync(bucketName, CoercePath(path), cancellationToken: cancellationToken);
